...
router.put("/:id", async (req, res) => {
  try {
    ...
    if (existingSlip.status === "completed") {
      return res
        .status(400)
        .json({ message: "Completed duty slips cannot be modified" });
    }
    ...
    if (updateData.status === "completed") {
      if (
        !updateData.startKM ||
        !updateData.endKM ||
        !updateData.startKMPhoto ||
        !updateData.endKMPhoto
      ) {
        return res
          .status(400)
          .json({ message: "Missing required fields for completion" });
      }
      if (!updateData.startTime || !updateData.endTime) {
        return res
          .status(400)
          .json({ message: "Start and end times are required for completion" });
      }
    }
